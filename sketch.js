...
function draw() {
  ...
  if(keyDown(LEFT_ARROW)){
    updateHeight(-10,0);
    //1 //add the animation of balloon [use balloonImage2]
    balloon.addAnimation ("hotairballoon",balloonImage2)
  }
  else if(keyDown(RIGHT_ARROW)){
    updateHeight(10,0);
    //2 //add the animation of balloon [use balloonImage2]
    balloon.addAnimation("hotairballoon",balloonImage2)
  }
  else if(keyDown(UP_ARROW)){
    updateHeight(0,-10);
 //3 //add the animation of balloon [use balloonImage2]
    balloon.addAnimation("hotairballoon",balloonImage2)
    balloon.scale=balloon.scale -0.005;
  }
  else if(keyDown(DOWN_ARROW)){
    updateHeight(0,+10);
  //4 //add the animation of balloon [use balloonImage2]
     balloon.addAnimation("hotairballoon",balloonImage2)
    balloon.scale=balloon.scale+0.005;
  }
